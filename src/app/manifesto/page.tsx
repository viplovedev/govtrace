...
import { MANIFESTO_PROMISES, REPRESENTATIVES, CURRENT_USER, computeRatingWeight } from '@/lib/data'
...
export default function ManifestoPage() {
  ...
  const selectableReps = REPRESENTATIVES.filter((r) => r.level !== 'ward')
  const rep            = selectableReps.find((r) => r.id === manifestoRepId) ?? selectableReps[0]

  const repPromises    = MANIFESTO_PROMISES.filter((p) => p.repId === rep?.id)
  const deliveredCount = repPromises.filter((p) => p.status === 'delivered').length
  const partialCount   = repPromises.filter((p) => p.status === 'partial').length
  const notCount       = repPromises.filter((p) => p.status === 'not').length
  const allocatedCount = repPromises.filter((p) => p.status === 'allocated').length

  function handleVerify(promiseId: string, verdict: VerificationVerdict) {
          ...
          <div className="p-4">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {selectableReps.map((r) => {
                const isActive   = r.id === (manifestoRepId || rep?.id)
