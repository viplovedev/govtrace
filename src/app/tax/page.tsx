...
import { getCurrentUser, getCitizens } from '@/lib/queries'
import { computeVoteWeights, computeRatingWeight } from '@/lib/data'
...
export default function TaxPage() {
  ...
  const [user, setCitizen]     = useState<Citizen | null>(null)
  const [citizens, setCitizens] = useState<Citizen[]>([])
  const [loading, setLoading]  = useState(true)

  useEffect(() => {
    Promise.all([getCurrentUser(), getCitizens()]).then(([u, cs]) => {
      if (u) setCitizen(u)
      setCitizens(cs)
      setLoading(false)
    })
  ...
  if (loading || !user) {
    return (
      <div>
        <PageTitleBar title="Tax Breakdown" subtitle="Loading…" />
  ...
  const weights      = computeVoteWeights(user.totalTax)
  const ratingWeight = computeRatingWeight(user.totalTax)
    ...
    <div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 24 }}>
        ...
        <div className="grid gap-3 mb-6" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
          <StatCard label="Total Tax Paid"    value={isPrivate ? formatCrore(user.totalTax)    : '██████'} accentColor="#FF9933" subtext="Direct + Indirect" />
          <StatCard label="Direct Tax (IT)"   value={isPrivate ? formatINR(user.directTax)     : '██████'} accentColor="#000080" subtext={`Bracket: ${user.taxBracket}`} />
          <StatCard label="Indirect Tax (GST)" value={isPrivate ? formatINR(user.indirectTax) : '██████'} accentColor="#138808" subtext="Estimate on consumption" />
          <StatCard label="Annual Income"     value={isPrivate ? formatCrore(user.annualIncome) : '██████'} accentColor="#D32F2F" subtext={user.occupationType} />
        </div>
                ...
                <tbody>
                  ...
                  <tr style={{ borderTop: '2px solid #e0e4ec', background: '#f5f7fa' }}>
                    ...
                    <td style={{ padding: '8px 0', fontWeight: 700, color: '#000080', fontSize: 14 }}>
                      {isPrivate ? formatINR(user.directTax) : '██████'}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div className="gt-card">
            ...
            <div className="p-4">
              ...
              {INDIRECT_BREAKDOWN.map((row) => {
                const pct = (row.estimate / user.indirectTax) * 100
                return (
              ...
              <div className="flex justify-between pt-2" style={{ borderTop: '0.5px solid #e0e4ec' }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: '#138808' }}>{isPrivate ? formatINR(user.indirectTax) : '██████'}</span>
              </div>
            </div>
          </div>
              ...
              {[
                ...
                <div key={label} className="mb-4">
                  ...
                  <p style={{ fontSize: 10, color: '#9ca3af', marginBottom: 4 }}>
                    {isPrivate ? formatCrore(user.totalTax) : mask(formatCrore(user.totalTax))} ÷ {formatCrore(pool)} × 100
                  </p>
                  <div style={{ height: 4, background: '#f5f7fa', borderRadius: 2 }}>
