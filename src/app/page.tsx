...
import {
  getCurrentUser, getBills, getSpendingWithLineItems, getManifestoPromises,
} from '@/lib/queries'
import { computeVoteWeights, formatCurrency } from '@/lib/data'
import { formatPct } from '@/lib/utils'
import type { Citizen, Bill, SpendingItem, ManifestoPromise } from '@/lib/types'
...
export default function DashboardPage() {
  ...
  if (loading || !user) {
    return (
      <div>
        <PageTitleBar title="Dashboard" subtitle="Loading…" />
  ...
  const totalSpent = spending.reduce((a, b) => a + b.amount, 0)
  ...
  return (
    <div>
      <PageTitleBar
        title="Dashboard"
        subtitle={`Welcome back, ${user.name} · FY 2024–25`}
      />
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 24 }}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
          <StatCard
            ...
            value={formatCurrency(user.totalTax, true)}
            subtext="Direct + Indirect · FY 2024–25"
            accentColor="#FF9933"
          />
          <StatCard
            label="Direct Tax"
            value={formatCurrency(user.directTax, true)}
            subtext={`Income Tax · ${user.taxBracket} slab`}
            accentColor="#000080"
          />
          <StatCard
            label="Indirect Tax"
            value={formatCurrency(user.indirectTax, true)}
            subtext="GST estimate on consumption"
            accentColor="#138808"
          />
          <StatCard
            label="Annual Income"
            value={formatCurrency(user.annualIncome, true)}
            subtext={user.occupationType}
            accentColor="#D32F2F"
          />
        </div>
              ...
              <div className="p-4">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between">
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, fontFamily: 'monospace' }}>{user.maskedPanId}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600 }}>{user.taxBracket}</span>
                  </div>
                  <div className="flex justify-between">
                    <span style={{ fontSize: 12, color: '#6b7280' }}>Occupation</span>
                    <span style={{ fontSize: 12, fontWeight: 600 }}>{user.occupationType}</span>
                  </div>
                </div>
              </div>
            </div>
